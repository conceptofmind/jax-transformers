...
from jax_transformers.autoregressive_wrapper import AutoregressiveWrapper
...
def max_neg_value(tensor):
    return -jnp.finfo(tensor.dtype).max
...
class TokenEmbedding(hk.Module):
    def __init__(self, dim, num_tokens, l2norm_embed = False):
        ...
        self.emb = hk.Embed(num_tokens, dim)
...
class AbsolutePositionalEmbedding(hk.Module):
    def __init__(self, dim, max_seq_len, l2norm_embed = False):
        super().__init__()
        self.scale = dim ** -0.5 if not l2norm_embed else 1.
        self.max_seq_len = max_seq_len
        self.l2norm_embed = l2norm_embed
        self.emb = hk.Embed(max_seq_len, dim)
...
class FixedPositionalEmbedding(hk.Module):
    def __init__(self, dim):
        super().__init__()
        self.inv_freq = 1. / (10000 ** (jnp.arange(0, dim, 2).float() / dim))

    def __call__(self, x, pos = None, seq_dim = 1, offset = 0):
        if not exists(pos):
            pos = jnp.arange(x.shape[seq_dim])

        pos = pos.type_as(self.inv_freq) + offset
        sinusoid_inp = pos.unsqueeze(-1) * self.inv_freq
        emb = jnp.concatenate((jnp.sin(sinusoid_inp), jnp.cos(sinusoid_inp)), axis = -1)
        return emb

class RelativePositionBias(hk.Module):
    def __init__(self, scale, causal = False, num_buckets = 32, max_distance = 128, heads = 8):
        ...
        self.relative_attention_bias = hk.Embed(num_buckets, heads)
    ...
    def __call__(self, qk_dots):
        i, j, _ = *qk_dots.shape[-2:], _
        q_pos = jnp.arange(i, dtype = torch.long)
        k_pos = jnp.arange(j, dtype = torch.long)
